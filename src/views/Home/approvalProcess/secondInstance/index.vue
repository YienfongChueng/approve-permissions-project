      ...
      <el-table-column
        ...
        <template v-slot="{ row }">
          ...
          <div v-else-if="item.key === 'result'">
            <el-tag type="success" size="mini" v-if="row[item.key] === 'pass'">二审通过</el-tag>
            <el-tag type="danger" size="mini" v-else-if="row[item.key] === 'reject'">二审驳回</el-tag>
...
import { approveTwoList, approveTwoPass, approveTwoReject } from '@/api/modules/approve.js'
export default {
  components: { Dialog },
  data() {
    return {
  ...
  methods: {
    getList() {
      approveTwoList(this.queryForm).then(res => {
    ...
    async handleRequest() {
      ...
      if (this.passOrRejectType === '通过') {
        res = await approveTwoPass({ app_id, app_type, id })
      } else {
        res = await approveTwoReject({ app_id, app_type, id, content })
      }
      if (res.code === 20000) {
