  ...
  <div>
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template v-slot="{ row }">
          ...
          <div v-else-if="item.key === 'result'">
            <el-tag type="success" size="mini" v-if="row[item.key] === 'pass'">一审通过</el-tag>
            <el-tag type="danger" size="mini" v-else-if="row[item.key] === 'reject'">一审驳回</el-tag>
      ...
      <el-table-column label="操作" width="250px">
        <template v-slot="{row}">
          <el-button :disabled="!!row.result" size="small" type="success" @click="handleSubmit(row, '通过')">通过</el-button>
          <el-button :disabled="!!row.result" size="small" type="danger" @click="handleSubmit(row, '驳回')">驳回</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页 -->
...
import { approveOneList, approveOnePass, approveOneReject } from '@/api/modules/approve.js'
export default {
  components: { Dialog },
  data() {
    return {
  ...
  methods: {
    getList() {
      approveOneList(this.queryForm).then(res => {
    ...
    async handleRequest() {
      ...
      if (this.passOrRejectType === '通过') {
        res = await approveOnePass({ app_id, app_type, id })
      } else {
        res = await approveOneReject({ app_id, app_type, id, content })
      }
      if (res.code === 20000) {
